...
Rel_D(devEnv, deployment, "Builds/Runs", "fprime-util + zephyr sdk via cmake")
...
- *Component-Based Design*: F' enables robust, modular components with formal interfaces
- *Hardware Abstraction*: Zephyr provides consistent APIs across different hardware platforms
- *Cross-Platform Support*: Applications can be ported to different boards with minimal changes
- *Real-Time Capabilities*: Zephyr RTOS delivers deterministic timing and scheduling
- *Development Tools*: Integrated build system and ground data system simplify development
- *Flight Heritage*: F' has been used in multiple NASA flight projects
