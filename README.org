...
2. *Setup the docker environment*
   ...
   *This may take a while (especially if using WSL) so take the time to complete other steps or otherwise get familiarize with the rest of the project.*
   ...
   ❯ ./run.sh build docker --clean
   Running repo check
   deps/fprime commit 12cb7105c1dee550d659302355aa21725abea876 not found in remote repository. Continue? (y/n) y
   ...
   #2 [zephyr auth] library/ubuntu:pull token for registry-1.docker.io
   #2 DONE 0.0s

   #3 [zephyr internal] load metadata for docker.io/library/ubuntu:24.04
   #3 DONE 0.4s

   #4 [zephyr internal] load .dockerignore
   #4 transferring context: 2B done
   #4 DONE 0.0s

   #5 [zephyr base 1/2] FROM docker.io/library/ubuntu:24.04@sha256:72297848456d5d37d1262630108ab308d3e9ec7ed1c3286a32fe09856619a782
   #5 CACHED

   #6 [zephyr internal] load build context
   #6 transferring context: 1.75kB done
   #6 DONE 0.0s
   ...
